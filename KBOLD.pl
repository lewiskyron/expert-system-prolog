...
% Study Spot: gecko_coffeehouse
study_spot(gecko_coffeehouse).
description(gecko_coffeehouse, "Trendy coffee shop with a full menu and acclaimed coffee selection. ").
address(gecko_coffeehouse, "49 Bethnal Grn Rd, London E1 6LA").
website(gecko_coffeehouse, "https://geckocoffee.house/pages/menu").
type(gecko_coffeehouse, cafe).
study_spot_preference(gecko_coffeehouse, paid).
coffee_available(gecko_coffeehouse, y).
food_available(gecko_coffeehouse, y).
meal_type(gecko_coffeehouse, full_meal).
meal_type(gecko_coffeehouse, snack_pastry).
wifi_needed(gecko_coffeehouse, y).
plug_needed(gecko_coffeehouse, y).
wheelchair_accessible(gecko_coffeehouse, y).
stars_rating(gecko_coffeehouse, 4.5).
walking_distance(gecko_coffeehouse, yes).
travel_distance(gecko_coffeehouse, 2.1).
closing_time_weekday(gecko_coffeehouse, 18:00 - 21:00).
closing_time_weekend(gecko_coffeehouse, after_21).
...
description(national_art_library_, "The UKs most comprehensive public reference collection of literature on the fine and decorative arts, including books, journals and much more.").
...
description(senate_house_library_, "The central library for the University of London. One of the UKs largest academic libraries for arts, humanities & social sciences.").
...
description(hilton_london_tower_bridge, "Renowned hotel near Londons central tourist destinations, featuring ample seating and coffee. ").
...
% Predicate to find a study spot that matches all user criteria
find_study_spot(StudySpot) :-
    study_spot(StudySpot),

    % Handle study spot preference (free, paid, no preference)
    (answer(study_spot_preference, free) -> study_spot_preference(StudySpot, free);
     answer(study_spot_preference, paid) -> study_spot_preference(StudySpot, paid);
     true),  % No preference

    % Coffee availability
    (answer(coffee_available, yes) -> coffee_available(StudySpot, y); true),

    % Food availability
    (answer(food_available, yes) -> food_available(StudySpot, y); true),

    % Meal type (full meal, snack/pastry, no preference)
    (answer(meal_type, full_meal) -> meal_type(StudySpot, full_meal);
     answer(meal_type, snack_pastry) -> meal_type(StudySpot, snack_pastry);
     true),  % No preference

    % Walking distance
    (answer(walking_distance, yes) -> walking_distance(StudySpot, yes); true),

    % Check if the travel distance matches user preference (convert users choice to applicable distance if needed)
    % Assume 'less_than_5km' and 'more_than_5km' are handled by a separate logic to compare with 'travel_distance' facts

    % Plug needed
    (answer(plug_needed, yes) -> plug_needed(StudySpot, y); true),

    % WiFi needed
    (answer(wifi_needed, yes) -> wifi_needed(StudySpot, y); true),

    % Minimum stars rating (ensure the spots rating meets or exceeds the users minimum requirement)
    answer(minimum_stars, MinStars),
    stars_rating(StudySpot, Rating),
    Rating >= MinStars,

    % Wheelchair accessible
    (answer(wheelchair_accessible, yes) -> wheelchair_accessible(StudySpot, y); true),

    % Handle closing time based on day preference and users closing time requirement
    % This part will need custom logic to compare the users preference with the study spots closing times
    true.  % Placeholder for actual closing time logic
